...
func main() {
    ...
    fmt.Println("db init success ", DB)

    // CreateTable(DB)
    // InsertData(DB, "test", "11999")
    // QueryOne(DB)
    // QueryMuli(DB)
    // UpdateData(DB)
    DeleteData(DB)
...
func CreateTable(DB *sql.DB) {
    ...
    fmt.Println("create table succssed")
}

func InsertData(DB *sql.DB, uname string, pwd string) {
    result, err := DB.Exec("insert iNTO users(username, password) values(?,?)", uname, pwd)
    if err != nil {
        fmt.Printf("Insert data failed, err: %v", err)
        return
    }
...
func QueryOne(DB *sql.DB) {
    user := new(User)
    row := DB.QueryRow("select id, username, password from users where username=?", "test")
    if err := row.Scan(&user.Id, &user.Username, &user.Password); err != nil {
...
func QueryMuli(DB *sql.DB) {
    user := new(User)
    rows, err := DB.Query("select id, username, password from users where username = ?", "test")
    ...
    for rows.Next() {
        ...
        fmt.Println("scan successed: ", *user)
    }
}

func UpdateData(DB *sql.DB) {
    result, err := DB.Exec("UPDATE users set password=? where id=?", "111", 1)
    if err != nil {
        fmt.Printf("Insert failed, err: %v\n", err)
        return
    }

    fmt.Println("update data successed:", result)
    rowsAffected, err := result.RowsAffected()
    if err != nil {
        fmt.Printf("Get rowsAffected failed, err: %v\n", err)
        return
...
func DeleteData(DB *sql.DB) {
    result, err := DB.Exec("delete from users where username=?", "test")
    if err != nil {
        fmt.Printf("Insert failed, err: %v\n", err)
        return
    }

    fmt.Println("delete data successed: ", result)
